...
<div class="article_main">
    <div class="rich_media_area_primary">
        ...
        <el-button type="primary" @click="finishReading">提交阅读记录 </el-button>
        <el-button type="primary" @click="expressOpinion">发表见解文章</el-button>
...
export default {
    ...
    methods:{
        finishReading () {  //完成阅读
            ...
            this.time=(this.nowtime-this.startTime)/1000;
            if(this.isfinish===1){
                
                if (this.isOvertime) {
                    this.isfinish=0;
                    addArticleCountOnce({articleid:this.$store.getters.artcleInfo.articleid}).then(data=>{
                        console.log(data);
                    });
                    const readLog = {
                            userid: this.$store.getters.userInfo.userid,
                            articleid:this.$store.getters.artcleInfo.articleid ,
                            duration: this.time,//单位秒
                    };
                    console.log(readLog);
                    newReadLog(readLog).then(data=>{
                    console.log(data);
                    });
                    this.$message.success('成功提交记录');
                    
                }else{
                    this.$message.error('请看完该文章');
                }
            }else{
                this.$message.success('你已提交记录');
            }
        },
        expressOpinion(){       //发表见解
            if (!this.isOvertime) {
                this.$message.error('请先看完该文章');
            }else{
                this.$router.push({
                path: "/case-management/case_add",
                });
            }
        },
        signOut(){              //退出
            let message="";
            if (!this.isOvertime) {
                message="你还未读完该文章,确认要退出？"
            }else{
                if(this.isfinish===1){
                    message="你还未提交阅读记录，是否要退出？"
                }else{
                    message="阅读记录已提交,可以退出"
                }
            }
                this.$confirm( 
                message,
